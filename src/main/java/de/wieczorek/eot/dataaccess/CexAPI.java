...
public class CexAPI {
	...
	private String signature() {
		...
		String message = new String(session.getNonce() + session.getUsername() + session.getApiKey());
		Mac hmac = null;

		try {
			hmac = Mac.getInstance("HmacSHA256");
			SecretKeySpec secret_key = new SecretKeySpec(session.getApiSecret().getBytes("UTF-8"), "HmacSHA256");
			...
			try {
				...
				if (param != "" || auth) {
					...
					if (auth) {
						...
						String tNon = String.valueOf(session.getNonce());
	 ...
	 */
	public String lastPrice(ExchangableType major, ExchangableType minor) {
	 ...
	 */
	public String chart(ExchangableType major, ExchangableType minor, int hours, int count) {
	...
	public String ohclv(ExchangableType major, ExchangableType minor, String timestamp) {
